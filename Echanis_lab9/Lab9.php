...
<body>
    ...
    <?php
        ...
        // TODO: 1.
        function computePizzaTotal($pizzaType, $selectedToppings, $quantity) {
            global $pizzaMenu, $toppingsMenu;

            // Base price from pizza type
            $basePrice = $pizzaMenu[$pizzaType];

            // Add up all selected toppings
            $toppingsTotal = 0;
            ...
            // Multiply total per pizza by quantity
            $grandTotal = ($basePrice + $toppingsTotal) * $quantity;
            return $grandTotal;
        }
    ?>
            ...
            <div class="card">
                ...
                <?php
                    if (isset($_POST['order'])) {
                        ...
                        // TODO: 6. 
                        echo '<div class="summary-item"><span>👤 Customer</span><strong>' . $customer . '</strong></div>';
                        echo '<div class="summary-item"><span>🍕 Pizza</span><strong>' . $selectedPizza . '</strong></div>';

                        // Display each topping or "None" if empty
                        if (!empty($selectedToppings)) {
                            echo '<div class="summary-item"><span>🧀 Toppings</span><strong>' . implode(', ', $selectedToppings) . '</strong></div>';
                        } else {
                            echo '<div class="summary-item"><span>🧀 Toppings</span><strong>None</strong></div>';
                        }

                        echo '<div class="summary-item"><span>🔢 Quantity</span><strong>' . $quantity . '</strong></div>';
                ...
                <?php
                    ...
                    if (isset($_POST['order'])) {
                        ...
                        for ($i = 1; $i <= $quantity; $i++) {
                            $toppingText = !empty($selectedToppings) ? ' + ' . implode(', ', $selectedToppings) : '';
                            echo '<div class="order-item">';
                            echo '  🍕 Order #' . $i . ' — ' . $customer . ': ' . $selectedPizza . $toppingText;
                            echo '</div>';
                        }
                    } else {
                        echo "<p style='text-align: center;'>Submit an order to see the counter.</p>";
                    }
                ?>
            </div>
        </div>
